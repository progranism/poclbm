...
#define P1(n) ((rot(W[(n)-2],15)^rot(W[(n)-2],13)^((W[(n)-2])>>10U)))
#define P2(n) ((rot(W[(n)-15],25)^rot(W[(n)-15],14)^((W[(n)-15])>>3U)))
#define P3(x)  W[x-7]
#define P4(x)  W[x-16]


//Partial Calcs for constant W values
#define P1C(n) ((rot2(ConstW[(n)-2],15)^rot2(ConstW[(n)-2],13)^((ConstW[(n)-2])>>10U)))
#define P2C(n) ((rot2(ConstW[(n)-15],25)^rot2(ConstW[(n)-15],14)^((ConstW[(n)-15])>>3U)))
#define P3C(x)  ConstW[x-7]
#define P4C(x)  ConstW[x-16]

//SHA round with built in W calc
#define sharoundW(n) Barrier(n); Vals[(3 + 128 - (n)) % 8] += t1W(n); Vals[(7 + 128 - (n)) % 8] = t1W(n) + t2(n);  

//SHA round without W calc
#define sharound(n)  Barrier(n); Vals[(3 + 128 - (n)) % 8] += t1(n); Vals[(7 + 128 - (n)) % 8] = t1(n) + t2(n);
...
//Dummy Variable to prevent compiler from reordering between rounds
	...
	W[3] = base + (uint)(get_local_id(0)) * 2 + (uint)(get_group_id(0)) * (WORKSIZE * 2);
	uint r = rot(W[3].x,25)^rot(W[3].x,14)^((W[3].x)>>3U);
	//Since only the LSB is opposite between the two nonces, we can save an instruction by flipping the 2 bits in W18 rather than the 1 bit in W3
	W[18] = PreW20 + (uint2)(r,r ^ (0x2004000U));
	
#else
	W[3] = base.x + get_local_id(0) * 2 + get_group_id(0) * (WORKSIZE * 2);
	uint r = rot(W[3],25)^rot(W[3],14)^((W[3])>>3U);
	W[18] = PreW20 + r;
#endif
//the order of the W calcs and Rounds is like this because the compiler needs help finding how to order the instructions
	...
	sharoundW(50);
	sharoundW(51);
//for some reason, this is faster than using all sharoundW...
	W(52);
	sharound(52);
	W(53);
	sharound(53);
	...
	//sharound(64 + 0);
	const u Temp = (0xb0edbdd0 + K[0]) +  W[64];
	Vals[7] = Temp + 0x08909ae5U;
	Vals[3] = 0xa54ff53a + Temp;
	...
	W[64 + 24] = P1(64 + 24) + P3(64 + 24) + P4C(64 + 24);
	sharoundC(64 + 9);
	sharoundC(64 + 10);
	...
	W(64 + 56);
	sharound(64 + 56);
	//adding the constants up here actually decreases the number of instructions... *shrug*
	//Also note that the constants have been kept separate for readability, the compiler combines them anyway
	u v = W[117] + W[108]  + (K[60] + H[7]) + Vals[3] + Vals[7] + P2(124);
	W(64 + 57);
	...
	v += Ch((Vals[0] + Vals[4]) + (K[59] + W(59+64))  + s1(64+59)+ ch(59+64),Vals[1],Vals[2]);
	v += S1((Vals[0] + Vals[4]) + (K[59] + W(59+64))  + s1(64+59)+ ch(59+64));
	
#ifdef VECTORS
	uint nonce = W[3].x + min(1U,v.x);
	if ((min(v.x,v.y)) == 0)
	{
